...
import mx.com.gentera.SearchClaim;
...
public class SearchClaimEndpoint implements SearchClaim{
	
	/**
	 * Instancia encargada de generar el log de la aplicacion
	 */
	private static final Logger logger = LoggerFactory.getLogger(SearchClaimEndpoint.class);
	...
	@Override
	public SearchClaimsRespMsgDataType claimsSearch(String partnerID, String businessPartnerID, String claimID,
			String certificate) {
		logger.info("Se recibio una peticion del id {} para el  servicio {}",businessPartnerID ,"SearchClaim");
		...
		try {
			callSPResponse = persistenceService.executeFunction_SIMA(searchClaimEndpointBean.getStoredProcedureName(), values, 
			...
			logger.error("Error en el consumo de servicio SearchClaimEndpoint: "+e.getMessage());
		}
