...
@Component
@StepScope
public class SendFilesRoboSPTaskletStep implements Tasklet, StepExecutionListener {
	private static final Logger log = LoggerFactory.getLogger(SendFilesRoboSPTaskletStep.class);
	...
	public RepeatStatus execute(StepContribution contribution, ChunkContext chunkContext) throws IOException,
		...
		this.lstFileItem = this.serverService.getLstFileItem(this.jobPropertiesBean.storedProcedureName, values, 1);
		log.info("Archivos encontrados: {}", Integer.valueOf(this.lstFileItem.size()));
		chunkContext.getStepContext().getStepExecution().getJobExecution().getExecutionContext().put("lstFileItem",
				this.lstFileItem);
		log.info("El paso obtener info de db ha finalizado");
