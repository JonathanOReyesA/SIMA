...
@Component
@StepScope
public class InsurancePEReader implements ItemStreamReader<String> {
	private static final Logger log = LoggerFactory.getLogger(InsurancePEReader.class);
	...
	public void beforeStep(StepExecution stepExecution) throws IOException {
		...
		stepExecution.getJobExecution().getExecutionContext().put("fileReader", this.resource.getFilename());
	...
	private void afterStep(StepExecution stepExecution) {
		log.info("Finalizo la lectura del archivo");
		this.delegate.close();
		this.resource = null;
	}
